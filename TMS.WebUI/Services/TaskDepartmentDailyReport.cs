...
{
    public class TaskDepartmentDailyReport
    {
        ...
        private ExcelPackage _reportExcelPck;
        public TaskDepartmentDailyReport(DateTime day, Department dep)
        {
            _reportTime = day;
            _dep = dep;
            _reportPath = HostingEnvironment.MapPath(WebConfigurationManager.AppSettings["ReportDirectoryRoot"]);
            _reporttemplateFile = HostingEnvironment.MapPath(WebConfigurationManager.AppSettings["DailyReportTemplate"]);
        ...
        public bool prepareReport()
        {
            ...
            string strDate = String.Format("{0:dd-MM-yyyy}", _reportTime);
            strTempFile = Path.Combine(_tempPath, "Báo cáo công văn ngày " + strDate + " " + _dep.DeptName + ".xlsx");
        ...
        public string loadReportFile(List<TaskReportRecordModel> data, List<TaskReportRecordModel> dataPrev)
        {
            ...
            try
            {
                using (_reportExcelPck = new ExcelPackage(fileinfo))
                {
                    ...
                    reporttitle = reporttitle.Replace("[day]", _reportTime.Day.ToString()).Replace("[month]", _reportTime.Month.ToString()).Replace("[year]", _reportTime.Year.ToString());
                    ...
                    foreach (TaskReportRecordModel item in data)
                    {
                        ...
                        if (item.CreationDate.Day == _reportTime.Day && item.CreationDate.Month == _reportTime.Month && item.CreationDate.Year == _reportTime.Year)
                    ...
                    foreach (TaskReportRecordModel item in dataPrev)
                    {
                        ...
                        excelWorksheet.Cells[startingrow + index + groupcount, 2].Value = item.Title;
                        excelWorksheet.Cells[startingrow + index + groupcount, 3].Value = item.AssigneeName;
                        excelWorksheet.Cells[startingrow + index + groupcount, 4].Value = item.LastUpdateDate.ToString("dd-MM-yyyy");
